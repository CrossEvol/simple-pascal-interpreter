...
class ModuleSymbolTable(ScopedSymbolTable):
    ...
    def import_module_symbols(
        self, module_name: str, symbol_table: ScopedSymbolTable
    ) -> None:
        """
        ...
        for symbol_name, symbol in symbol_table._symbols.items():
            if (
                hasattr(symbol, "visibility")
                and symbol.visibility == VisibilityLevel.INTERFACE
            ):
    ...
    def resolve_qualified_name(
        ...
        if module_name in self.imported_modules:
            return self.imported_modules[module_name].lookup(
                symbol_name, current_scope_only=True
            )
        return None
    ...
    def lookup_with_visibility(
        self, name: str, requesting_module: str | None = None
    ) -> Symbol | None:
        """
    ...
    def insert_with_visibility(
        self, symbol: Symbol, visibility: VisibilityLevel
    ) -> None:
        """
    ...
    def get_implementation_symbols(self) -> Dict[str, Symbol]:
        ...
        impl_symbols = {}
        for name, symbol in self._symbols.items():
            if hasattr(symbol, "visibility") and symbol.visibility in [
...
class ModuleRegistry:
    ...
    def resolve_dependencies(self, module_name: str) -> List[str]:
        ...
        def visit(name: str) -> None:
            if name in temp_visited:
                ...
                # Generate helpful suggestions for resolving the circular dependency
                suggestions = self.get_circular_dependency_suggestions(cycle)

                from src.error import CircularDependencyError

                raise CircularDependencyError(cycle, suggestions)
    ...
    def check_circular_dependencies(self, module_name: str) -> bool:
        ...
        This method performs a depth-first search to detect cycles in the dependency graph.
        It's more efficient than resolve_dependencies for just checking cycles.
        ...
        def has_cycle(current_module: str) -> bool:
            """
            Recursive helper function to detect cycles using DFS.

            Args:
                current_module: Current module being visited

            Returns:
                True if a cycle is detected, False otherwise
            """
            if current_module in recursion_stack:
                return True
            ...
            recursion_stack.add(current_module)

            # Check all dependencies of current module
            for dependency in self.dependency_graph.get(current_module, []):
                if has_cycle(dependency):
                    return True

            recursion_stack.remove(current_module)
            return False

        return has_cycle(module_name)

    def find_circular_dependency_chain(self, module_name: str) -> Optional[List[str]]:
        """
        Find and return the circular dependency chain if one exists.

        This method performs a depth-first search to find the exact cycle path
        in the dependency graph, which is useful for error reporting.

        Args:
            module_name: The module name to check for circular dependencies

        Returns:
            List of module names forming the circular dependency chain,
        ...
        def find_cycle(current_module: str) -> Optional[List[str]]:
            """
            Recursive helper function to find the cycle path.

            Args:
                current_module: Current module being visited

            Returns:
                List of modules in the cycle if found, None otherwise
            """
            if current_module in recursion_stack:
                # Found a cycle - extract the cycle from the path
                cycle_start = path.index(current_module)
                return path[cycle_start:] + [current_module]
        ...
        return find_cycle(module_name)

    def get_circular_dependency_suggestions(
        self, dependency_chain: List[str]
    ) -> List[str]:
        """
        Generate recovery suggestions for circular dependency errors.

        Args:
            dependency_chain: The circular dependency chain

        Returns:
            List of suggestion strings for resolving the circular dependency
        """
        suggestions = []
        ...
        # Remove the duplicate module at the end of the chain
        clean_chain = (
            dependency_chain[:-1]
            if dependency_chain[-1] == dependency_chain[0]
            else dependency_chain
        )

        suggestions.append("Circular dependency detected. Consider these solutions:")
        suggestions.append("")

        # Suggestion 1: Extract common functionality
        suggestions.append("1. Extract Common Functionality:")
        suggestions.append(
            f"   - Create a new module with shared functionality used by {clean_chain[0]} and {clean_chain[-1]}"
        )
        suggestions.append(f"   - Move common code to this new module")
        suggestions.append(
            f"   - Have both modules depend on the new module instead of each other"
        )
        suggestions.append("")

        # Suggestion 2: Merge modules
        if len(clean_chain) == 2:
            suggestions.append("2. Merge Modules:")
            suggestions.append(
                f"   - Consider merging {clean_chain[0]} and {clean_chain[1]} into a single module"
            )
            suggestions.append(f"   - This eliminates the circular dependency entirely")
            suggestions.append("")

        # Suggestion 3: Dependency inversion
        suggestions.append("3. Use Dependency Inversion:")
        suggestions.append(f"   - Define interfaces in a separate module")
        suggestions.append(
            f"   - Have {clean_chain[0]} depend on the interface instead of {clean_chain[-1]} directly"
        )
        suggestions.append(f"   - Implement the interface in {clean_chain[-1]}")
        suggestions.append("")

        # Suggestion 4: Restructure dependencies
        if len(clean_chain) > 2:
            suggestions.append("4. Restructure Dependencies:")
            dependency_path = " -> ".join(clean_chain)
            suggestions.append(
                f"   - Current path: {dependency_path} -> {clean_chain[0]}"
            )
            suggestions.append(
                f"   - Consider removing the dependency from {clean_chain[-1]} to {clean_chain[0]}"
            )
            suggestions.append(
                f"   - Or restructure the modules to create a hierarchical dependency tree"
            )
            suggestions.append("")

        # Suggestion 5: Use forward declarations
        suggestions.append("5. Use Forward Declarations (if supported):")
        suggestions.append(
            f"   - Use forward declarations in {clean_chain[0]} for types from {clean_chain[-1]}"
        )
        suggestions.append(
            f"   - Move the actual dependency to the implementation section"
        )
        suggestions.append("")
